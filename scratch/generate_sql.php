...
$sql = "-- 1. Masukkan posisi baru ke tabel posisi jika belum ada\n";
$sql .= "INSERT IGNORE INTO `posisi` (`nama`) VALUES ('ADMIN MARKETING & SOSMED');\n\n";

$sql .= "-- 2. Hapus data lama posisi ini jika ada agar tidak double\n";
$sql .= "DELETE FROM `bank_soal_sesi5` WHERE `posisi` = 'ADMIN MARKETING & SOSMED';\n\n";

$sql .= "-- 3. Masukkan 5 Studi Kasus ADMIN MARKETING & SOSMED ke bank_soal_sesi5 (Format Rapih dengan <br> & <strong>)\n";
$sql .= "INSERT INTO `bank_soal_sesi5` (`posisi`, `tipe`, `judul`, `deskripsi`, `pertanyaan`) VALUES\n";

$values = [];
foreach ($skData as $item) {
    $pEsc = addslashes($item['posisi'] ?? 'ADMIN MARKETING & SOSMED');
    $jEsc = addslashes($item['judul']);
    $dEsc = addslashes($item['deskripsi']);
    $qEsc = addslashes(json_encode($item['pertanyaan'], JSON_UNESCAPED_UNICODE));

    $values[] = "('$pEsc', 'studi_kasus', '$jEsc', '$dEsc', '$qEsc')";
}

$sql .= implode(",\n\n", $values) . ";\n";

file_put_contents(__DIR__ . '/../scratch/update_admin_marketing_clean.sql', $sql);
echo "Clean SQL File Generated Successfully!\n";
